...
# Removed the unsupported '--skip=cpu' flag.
ARG UV_SYNC_EXTRA_ARGS="--all-extras"
...
# Synchronize dependencies with caching.
RUN --mount=type=cache,target=/opt/app-root/src/.cache/uv,uid=1001 \
    uv sync --frozen --no-install-project --no-dev ${UV_SYNC_EXTRA_ARGS}
...
# Second synchronization using cache.
RUN --mount=type=cache,target=/opt/app-root/src/.cache/uv,uid=1001 \
    uv sync --frozen --no-install-project --no-dev ${UV_SYNC_EXTRA_ARGS}
